...
router.get('/', function(req, res){
    ...
    $referredClientPromise = Promise.resolve([]);
    if(req.query.extra && req.query.extra.external){
      $referrerIdentifier = req.query.extra.external.referrer_identifier;
      $referredClientPromise = referrerLogic.getAllReferredClients($referrerIdentifier);
    }

    $referredClientPromise
    .then(function(clients){

      clientNumbers = [];
      for(I=0; I < clients.length; I++){
        clientNumbers.push(clients[I]["mobile"]);
      }
      if(clientNumbers.length > 0){
        extraQuery["sender"] = {}
        extraQuery["sender"]["$in"] = clientNumbers;
      }
    })
    .then(function(){
